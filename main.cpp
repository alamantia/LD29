...
Thing * boxForPosition(int x, int y)
{
  for (auto i = things.begin(); i != things.end(); ++i) {
   ...
   pos_x = p->x / UNIT_WIDTH;
   if ((int)pos_x == x && (int) pos_y == y) {
     if ((int)pos_x != p->x) {
       // printf("... %i %i\n", (int)pos_x, p->x/UNIT_WIDTH);
     }
     return p;
   }
...
Thing * boxForPositionMatch(int x, int y)
{
 for (auto i = things.begin(); i != things.end(); ++i) {
    Thing *p = *i;
    if (p->type != LOL_BOX)
...
void matching_shit()
{
  ...
  for (int x = 0; x < BOX_WIDTH; x++)
    for (int y = 0; y < BOX_HEIGHT; y++) {
      grid[x][y] = boxForPositionMatch(x, y);
      gridTmp[x][y] = grid[x][y];
    }
    ...
    for (int x = 0; x < BOX_WIDTH; x++) {
      ...
      if (maybeKillTheseThings.size() >= 3) {
        for (auto it =  maybeKillTheseThings.begin(); it != maybeKillTheseThings.end(); ++it) {
          Thing *bye = *it;
          AddExplosion(bye->body->GetPosition().x*BOX_SCALE, bye->body->GetPosition().y*BOX_SCALE);
          things.remove(*it);
          delete bye;
          memset(&grid, 0, sizeof(grid));
          for (int x = 0; x < BOX_WIDTH; x++)
            for (int y = 0 ;y < BOX_HEIGHT; y++) {
              grid[x][y] = boxForPositionMatch(x, y);
              gridTmp[x][y] = boxForPositionMatch(x, y);
            }            
          last_match = SDL_GetTicks();
        }
      } 
    }
  }
